...
namespace WA.DiceGames
{
	...
	public class FourDiceGame
	{
		...
		public static FourDiceGame NewGame()
		{
			...
			while (game_Nember_Test < checktest)
			{
		...
		public int ComputePoint()
		{
			...
			for (var i = 0; i < _numbers.Length ; i++)_number[i] = _numbers[i];

            //後找出相同歸零
            for (var i = 0; i < _number.Length - 1; i++)
            {
				if (game_Nember_Test< checktest) {
            ...
            foreach (var i in _number)results += i;
			return  results;
		}
